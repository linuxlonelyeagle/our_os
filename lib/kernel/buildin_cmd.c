...
void buildin_pwd(uint32_t argc, char** argv ) {
   if (argc != 1) {
      put_str("pwd: no argument support!\n");
      return;
   } else {
	 ...
	 put_str("pwd: get current work directory failed.\n");
      }
   }
}
...
char* buildin_cd(uint32_t argc, char** argv) {
   if (argc > 2) {
      put_str("cd: only support 1 argument!\n");
      return NULL;
   }
   ...
   if (chdir(final_path) == -1) {
      put_str("cd: no such directory %s\n", final_path);
      return NULL;
   }
   return final_path;
}
...
void buildin_ls(uint32_t argc, char** argv) {
   ...
   while (arg_idx < argc) {
	 ...
	 if (!strcmp("-l", argv[arg_idx])) {         // 如果是参数-l
	    ...
	    put_str("usage: -l list all infomation about the file.\n-h for help\nlist all files in the current dirctory if no option\n"); 
	    return;
	 } else {	// 只支持-h -l两个选项
	    put_str("ls: invalid option %s\nTry `ls -h' for more information.\n", argv[arg_idx]);
	    return;
	 }
      } else {	     // ls的路径参数
	 if (arg_path_nr == 0) {
	    ...
	    put_str("ls: only support one path\n");
	    return;
	 }
      }
      arg_idx++;
   ...
   if (pathname == NULL) {	 // 若只输入了ls 或 ls -l,没有输入操作路径,默认以当前路径的绝对路径为参数.
	 ...
	 put_str("ls: getcwd for default path failed\n");
	 return;
      }
   } else {
...
void buildin_ps(uint32_t argc, char** argv) {
   if (argc != 1) {
      put_str("ps: no argument support!\n");
      return;
   }
   ps();
}
...
void buildin_clear(uint32_t argc, char** argv) {
   if (argc != 1) {
      put_str("clear: no argument support!\n");
      return;
   }
   clear();
}
...
int32_t buildin_mkdir(uint32_t argc, char** argv) {
   int32_t ret = -1;
   if (argc != 2) {
      put_str("mkdir: only support 1 argument!\n");
   } else {
      make_clear_abs_path(argv[1], final_path);
      /* 若创建的不是根目录 */
...
int32_t buildin_rmdir(uint32_t argc, char** argv) {
   int32_t ret = -1;
   if (argc != 2) {
      put_str("rmdir: only support 1 argument!\n");
   } else {
      make_clear_abs_path(argv[1], final_path);
   /* 若删除的不是根目录 */
      if (strcmp("/", final_path)) {
...
int32_t buildin_rm(uint32_t argc, char** argv) {
   int32_t ret = -1;
   if (argc != 2) {
      put_str("rm: only support 1 argument!\n");
   } else {
      make_clear_abs_path(argv[1], final_path);
   /* 若删除的不是根目录 */
      if (strcmp("/", final_path)) {
