...
void open_root_dir(struct partition* part) {
   root_dir.inode = inode_open(part, part->sb->root_inode_no);    // root_inode_no 是根目录所在的i结点号
   root_dir.dir_pos = 0;
}
 ...
 *2 root_dir所在的内存是低端1M之内,并非在堆中,free会出问题 */
   ...
   inode_close(dir->inode);   // 就是清除inode队列中的inode，然后释放空间,inode是在内核空间申请的
   sys_free(dir);
}
...
bool sync_dir_entry(struct dir* parent_dir, struct dir_entry* p_de, void* io_buf) {
   ...
   while (block_idx < 140) {  // 文件(包括目录)最大支持12个直接块+128个间接块＝140个块
	 ...
	 /* 每分配一个块就同步一次block_bitmap */
	    ...
	    all_blocks[12] = block_lba;   // 第0个间接扇区
